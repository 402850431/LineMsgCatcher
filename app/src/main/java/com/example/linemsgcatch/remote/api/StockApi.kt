...
class StockApi : BasicWebApi()  {
    ...
    fun getChart(symbolId: Int, resultListener: ResultListener) {
        ...
        createRequest(url, resultListener)
    }
...
/**
    ...
    fun getQuote(symbolId: Int, resultListener: ResultListener) {
        val url = "${ApiDataManager.baseUrl}/intraday/quote?symbolId=${symbolId}&apiToken=${ApiDataManager.apiToken}"
        createRequest(url, resultListener)
    }
...
/**
    ...
    fun getMeta(symbolId: Int, resultListener: ResultListener) {
        val url = "${ApiDataManager.baseUrl}/intraday/meta?symbolId=${symbolId}&apiToken=${ApiDataManager.apiToken}"
        createRequest(url, resultListener)
    }
...
//     * @param oddLot: 設置 true 回傳零股行情。預設值：false
    ...
    fun getDealt(symbolId: Int, limit:Int?, offset:Int?, /* oddLot:Boolean? = false, */ resultListener: ResultListener) {
        var url = "${ApiDataManager.baseUrl}/intraday/dealts?symbolId=${symbolId}&apiToken=${ApiDataManager.apiToken}&limit=50"
